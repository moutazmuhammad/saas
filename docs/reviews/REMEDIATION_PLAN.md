...
- [~] 🟠 **SCALE-001 / SCALE-003** Per-proxy lock around nginx write+`nginx -t`+reload; write-temp-then-atomic-rename. — **done (mock-tested; live-proxy verify pending deploy)**: new `_nginx_apply_vhost`/`_nginx_remove_vhost` SFTP the config to a temp OUTSIDE `sites-enabled`, then run `mv`→`nginx -t`→reload inside a host-side `flock /run/lock/saas-nginx.lock`, restoring the prior vhost (no reload) on `-t` failure. All 3 mutation sites (deploy/refresh/remove) routed through them. `TestNginxVhost` (3 tests) asserts the command shape; real nginx behaviour needs a proxy host.
