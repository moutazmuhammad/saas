...
### Scale / perf
- [x] 🟠 **PERF-001** Parallelize backups (bounded pool) — **done + tested**: `_cron_backup_all_instances` now enqueues one backup job per eligible instance (`channel='backup'`, `lock_key='instance:<id>'`, `idempotency_key='daily_backup:<id>:<date>'`) instead of running them serially in the cron thread. Completed the queue's concurrency model: immediate workers now **drain their channel** (`_worker_loop`/`_claim_next_in_channel`), so the per-channel cap (default 8) becomes a real worker pool → ~8 backups run in parallel (bounded), closing the nightly window at scale. Daily backups also gain durability (reaper) + alert/audit. Tests: worker-loop drains channel; claim is channel-scoped; cron enqueues only enabled/un-suspended instances; same-day re-run is idempotent. Full suite 0/226. *(Per-job timeout + lag metric not yet added.)*
...
- [x] 🟡 **PERF-008 / UX-006** SPA polling: backoff + pause-on-hidden + stop-on-auth — **done + tested (tsc/build)**: new `usePolling` hook (exponential backoff on error, pauses while tab hidden + resumes on focus, stops permanently on `auth_required`). ALL pollers migrated: dashboard (InstancesContext), Backups, Databases, Environments, DeploymentHistory, PerformanceHistory (history + live), InstanceDetail (status + live metrics). SPA `npm run build` clean. No JS test runner in repo → verification is tsc+build + logic review.
- [x] 🟢 **ARCH-004 hardening** queue per-channel concurrency cap — **done + tested**: the immediate worker spawned one thread per enqueue (unbounded → thundering-herd risk under bulk enqueue). Added a soft per-channel cap (`_should_spawn_now`/`_channel_concurrency`, default 8, override via `ir.config_parameter saas_master.job_channel_concurrency`); over-cap jobs stay pending and the cron drains them. Can't regress current behaviour (only bounds). `TestJobQueue.test_channel_concurrency_cap`; full suite 0/217.
- [x] 🟢 **ARCH-004 retry cutover** queue OWNS deploy retries + crash recovery — **done + tested**: deploy job now `max_attempts=max_deploy_retries+1`, `idempotent=True` (reaper requeues a dead deploy; `_do_deploy` is re-runnable — recover-stuck already relied on that, and `_allocate_servers` is a no-op once a host is assigned so retries hit the same host either way → no regression). `on_error` moved to `_finish_failed` so it fires only on TERMINAL failure (not per-retry). `_on_background_error`'s deploy path now goes straight to `failed` (no pending_provision re-queue → no double-retry; the two non-queue `'failed'` callers route through `_do_deploy_after_payment → action_deploy`, so they inherit queue retries). `_cron_recover_stuck_provisioning` now skips instances with a live (pending/running) job (defers to the reaper) and only rescues legacy run_in_background ops; `_cron_retry_pending_provision` re-scoped to capacity-waiting only (PROV-004 kept). Crons NOT deleted — reduced to their irreducible roles (capacity-waiting; legacy-op recovery). Tests: on_error-only-on-terminal, deploy enqueue max_attempts/idempotent, terminal→failed, recover-stuck-defers-to-queue. Full suite 0/220. **Live deploy smoke still recommended before relying on this in prod (harness torn down).**
- [x] 🟢 **ARCH-004** restart + redeploy on the queue — **done + tested**: `action_restart`/`action_redeploy` now `_enqueue(idempotent=True, max_attempts=2, on_error='_on_background_error')` instead of `run_in_background` → crash recovery via the reaper actually *completes* the op (vs recover-stuck merely reverting). stop/suspend/restore deliberately left on `run_in_background` (not in `_RECOVERABLE_OPERATIONS` / need restore health-gate); recover-stuck kept as backstop. `TestDeployViaQueue.test_action_restart_enqueues_idempotent_job` + `_redeploy_`; full suite 0/222.
