...
- [ ] **M2** — Snapshot line builder (`_snapshot_order_line`, ALWAYS qty 1 = one month) reused by both paths
- [ ] **M3** — Renewal includes the snapshot line **only when the snapshot is due on/before this renewal**, then advances the snapshot date by 1 month
