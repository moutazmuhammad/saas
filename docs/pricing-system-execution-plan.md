...
- [ ] **S2** — Repoint all callers (4× `main.py`, 1× `api.py`) to the engine. Add consistency test.
...
- [ ] **S7** — **Region pricing**: region model + per-region price multiplier + availability/default; customer selects region at instance create; allocation honors it; engine applies the region factor. (See §3.7.)
