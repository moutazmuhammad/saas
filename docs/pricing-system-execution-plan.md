...
- [x] **S8** — Pricing/create UI (S8a rates hidden · S8b tier cards · S8c region picker + proxy co-location). **S8a DONE:** unit rates no longer exposed — `/meta` strips `worker_price`/`storage_price_per_gb` (new `_public_plan_config`), removed from `PlanConfigMeta`; confirmed the SPA never used them (it prices via `hostingCalculate` → engine). `PlanBuilder` already takes a server-computed price (no client math). **S8b DONE:** `Hosting.tsx` renders `/tiers` (kind=hosting) as cards — recommended tier highlighted + `badge` (default "Most popular"), shared monthly/yearly toggle drives card prices via `config.cycle`, "Choose <tier>" → `/hosting/configure?workers&storage&billing`. Slider/`PlanBuilder` is behind a "Build a custom plan" toggle (`customize`), with "← Back to standard plans" to return. **Falls back to slider-only when `/tiers` returns `[]`** (none seeded yet ⇒ behaviour-neutral). Added `ApiTier` + `api.tiers(kind)` to `lib/api.ts`. `tsc --noEmit` + `vite build` clean. REMAINING: **S8c** region picker at create + pass region into create controllers/pricing + **proxy co-location** (region-aware domain selection — closes the 3-server rule). S8c is the riskiest (live signup/checkout) — do with care. **S8c DONE** (commit on `price`): region picker on the QWeb `/hosting/configure` funnel (every create path lands there; the SPA redirects in). Controller helpers `_resolve_region` (pick→default→None, behaviour-neutral) and `_region_domain_ok` (proxy co-location: domain usable in a region iff its `proxy_server_id` is in-region, or proxy-less/region-less ⇒ neutral). GET resolves region, prices via `engine.compute(region=...)`, filters domains to region-compatible, renders `<select name="region_id">` only when >1 active region (else hidden default); changing it reloads the funnel carrying non-sensitive fields (Git token kept out of URL) so price+domains recompute server-side. POST validates proxy co-location + region-aware infra (docker+db must exist in-region via `_region_match_domain`), passes `region=` to the plan, pins `vals['region_id']` ⇒ S7b co-locates all three servers. `region_id` carried through public signup (`registration._build_redirect_url` + hidden fields in the OTP templates) and the funnel's public redirect. Behaviour-neutral with 0/1 region. **Scope note:** region is on the hosting funnel only; the services custom flow still prices ×1.0 (region=None) — deliberate follow-up. No SPA region picker (region is chosen at the QWeb create step).
- [~] **S9** — SKIPPED by owner decision (2026-05-30): no seed tiers/regions; they're created in the admin UI when ready. System stays behaviour-neutral until then (no published tiers ⇒ slider only; ≤1 active region ⇒ no picker). The single seeded ×1.0 Default region (S7) and the `daily_snapshots` add-on (S5) remain — both behaviour-neutral.
- [x] **S10** — Regression pass + operator docs. All pricing-touching Python compiles (`saas_pricing`, `saas_addon`, `saas_region`, `saas_plan`, `saas_server`, `saas_instance`, `res_config_settings`, `main.py`, `registration.py`, `api.py`, engine test); all new/edited views + data XML well-formed; new models imported in `models/__init__`; manifest registers data+views; ACL grants user/manager/**public** read on `saas.addon`/`saas.region` (public read powers `/tiers` + `/regions`). New **`docs/pricing-admin-guide.md`** documents how to configure rates/limits, floors, named tiers, add-ons, regions + co-location, storage overage, snapshot policy, and how to run the engine test suite. **Pricing rebuild S1–S10 complete.**
