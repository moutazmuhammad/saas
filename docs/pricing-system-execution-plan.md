...
- [ ] **S7b** — Region ENFORCEMENT (deferred — provisioning-critical, do with clean reads): region-aware `_allocate_servers` placing proxy+docker+db **all in the chosen region** (co-location hard rule; a region is selectable only if it has capacity in all three roles; never cross-region fallback); pass the instance's `region_id` into pricing at the configure/checkout & upgrade sites so the correct multiplier is charged; store `region_id` on the instance at create. (Region pricing is NOT production-active until S7b + the S8 picker land.)
- [ ] **S8** — Hide unit rates from public API; SPA renders tiers (cards + recommended/default/badge from config) **and a region selector at create** (price reflects region live); slider calls calculate; remove client-side price math.
